...
export class AntigravityServer {
  ...
  constructor(
    ...
    this.uploadsDir = fs.existsSync(rootUploads)
      ? rootUploads
      : path.join(extensionPath, "uploads");
    this.publicDir = fs.existsSync(rootPublic)
      ? rootPublic
      : path.join(extensionPath, "public");
    this.state = {
      cdpConnections: [],
      lastSnapshot: null,
  ...
  private configureMiddleware() {
    ...
    this.app.use(
      express.urlencoded({ limit: `${MAX_UPLOAD_SIZE_MB}mb`, extended: true }),
    );
  ...
  private scoreTarget(target: { title?: string; url?: string }): number {
    ...
    if (title.includes("chat")) score += 30;
    if (url.includes("cockpit")) score += 40;
    if (url.includes("workbench")) score += 10;
    if (url.includes("jetski")) score += 10;
    if (title.includes("antigravity")) score += 15;
    if (url.startsWith("view://")) score += 20;
    if (url.startsWith("vscode-webview")) score += 25;
  ...
  private isChatTarget(target: { title?: string; url?: string }): boolean {
    ...
    // Specifically exclude our own UI
    if (url.includes("out/index.html") || title.includes("antigravity link"))
      return false;

    return (
      this.isWorkbenchTarget(target) ||
      title.includes("antigravity") ||
  ...
  private async initCDP(targetId?: string): Promise<void> {
    ...
    console.log(`🔍 [initCDP] Discovered ${instances.length} targets`);
    instances.forEach((i) =>
      console.log(`   - [${i.port}] "${i.title}" (${i.id})`),
    );
    ...
    if (!chosen) {
      ...
      const prioritizedChat = [...chatTargets].sort(
        (a, b) => this.scoreTarget(b) - this.scoreTarget(a),
      );
      chosen =
        prioritizedChat[0] ||
        [...instances].sort(
    ...
    if (chosen)
      console.log(
        `🎯 [initCDP] Chosen target: "${chosen.title}" (${chosen.id})`,
      );

    // Close existing
    this.state.cdpConnections.forEach((c) => {
      try {
        c.ws.close();
      } catch {}
    });
    ...
    if (!chosen) {
      console.warn(
        "⚠️ [initCDP] No suitable target found (e.g., chat/webview).",
      );
      return;
    }

    const prioritized = [...instances].sort(
      (a, b) => this.scoreTarget(b) - this.scoreTarget(a),
    );
    // Try chosen first, then fall back to others if connect fails
    const candidates = [
      chosen,
      ...prioritized.filter((t) => t.id !== chosen!.id),
    ];

    for (const candidate of candidates) {
      try {
        console.log(
          `🔗 [initCDP] Connecting to ${candidate.id} on port ${candidate.port}...`,
        );
        ...
        console.log(`✅ [initCDP] Connected successfully to ${candidate.id}`);
        break;
      } catch (err) {
        console.error(
          `❌ [initCDP] Connection failed for ${candidate.id}:`,
  ...
  private async updateSnapshot(): Promise<boolean> {
    ...
    const cdp = this.state.activeTargetId
      ? this.state.cdpConnections.find(
          (c) => c.id === this.state.activeTargetId,
        )
    ...
    try {
      ...
      if (snapshot && !snapshot.error) {
        console.log(
          `📸 [updateSnapshot] Captured snapshot from "${cdp.title}" (${snapshot.html.length} bytes)`,
        );
        ...
        if (hash !== this.state.lastSnapshotHash) {
          ...
          if (this.state.activePort)
            this.state.snapshotCache.set(this.state.activePort, snapshot);
        ...
        console.error(
          `❌ [updateSnapshot] Capture Error from "${cdp.title}":`,
          snapshot.error,
        );
      } else {
        console.warn(
          `⚠️ [updateSnapshot] captureSnapshot returned null for "${cdp.title}"`,
        );
      }
    } catch (err) {
  ...
  private configureRoutes() {
    ...
    router.post("/click", async (req, res) => {
      const { text, tag, x, y, selector } = req.body as {
        text?: string;
        tag?: string;
        x?: number;
        y?: number;
      ...
      try {
        ...
        res
          .status(404)
          .json({ error: "Could not find element to click in active target" });
      } catch (e) {
        res.status(500).json({ error: (e as Error).message });
      }
    });
    ...
    router.post("/upload", async (req, res) => {
      const { name, content, targetSelector } = req.body as {
        name: string;
        content: string;
      ...
      try {
        ...
        if (buffer.length > maxBytes) {
          return res
            .status(413)
            .json({ error: `File too large. Max ${MAX_UPLOAD_SIZE_MB}MB.` });
        }
        if (!fs.existsSync(this.uploadsDir))
          fs.mkdirSync(this.uploadsDir, { recursive: true });
        ...
        const injectionResult = await injectFile(
          activeCdp,
          targetPath,
          targetSelector,
        );
        if (injectionResult.ok) {
          ...
          res.json({
            ...
            path: targetPath,
            injected: false,
            reason: injectionResult.reason || "injection_failed",
          });
        }
      } catch (e) {
        res.status(500).json({ error: (e as Error).message });
    ...
    router.get("/debug/targets", async (_req, res) => {
      try {
        ...
        res.json({
          activeTargetId: this.state.activeTargetId,
          activePort: this.state.activePort,
          connected: this.state.cdpConnections.map((c) => ({
            title: c.title,
            url: c.url,
      ...
      try {
        if (this.useHttps) {
          ...
          if (fs.existsSync(certPath) && fs.existsSync(keyPath)) {
            ...
            const attrs = [
              { name: "commonName", value: "Antigravity Link Extension" },
            ];
            // @ts-ignore
        ...
        this.server.listen(this.port, async () => {
          ...
          const cleanIp =
            priorityMatch ||
            candidates.find(
              (c) => !/virtual|vbox|wsl|vpn|tailscale/i.test(c.name),
            ) ||
            candidates[0];
          ...
          try {
            ...
            console.log(
              "⚠️ Initial CDP connection failed, will keep polling...",
            );
          }
